        ...
        <form method="POST" action="{{ route('admin.products.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="5" required
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent @error('description') border-red-400 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input id="image" type="url" name="image" value="{{ old('image') }}" placeholder="https://..."
                        ...
                        <div>
                            <label class="flex items-center gap-2 text-sm font-medium text-gray-800">
                                <input type="checkbox" name="categories[]" value="{{ $root->id }}"
                                    {{ in_array($root->id, old('categories', [])) ? 'checked' : '' }}
                                ...
                                @foreach ($child->children as $grandchild)
                                    <label class="flex items-center gap-2 text-sm text-gray-600 ml-12 mt-1">
                                        <input type="checkbox" name="categories[]" value="{{ $grandchild->id }}"
                                            {{ in_array($grandchild->id, old('categories', [])) ? 'checked' : '' }}
                                            class="rounded border-gray-300 text-gray-900 focus:ring-gray-900">
                                        {{ $grandchild->title }}
                                    </label>
                                @endforeach
